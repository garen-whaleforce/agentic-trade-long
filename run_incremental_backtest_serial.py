...
def run_backtest(calendar_df: pd.DataFrame, max_samples: Optional[int] = None,
    ...
    for i, row in enumerate(calendar_df.itertuples(), 1):
        ...
        if i % CHECKPOINT_INTERVAL == 0 or i == total_samples:
...
def main():
    ...
    if args.test:
        print(f"\n🧪 測試模式: 處理 {args.test} 筆")
        run_backtest(calendar_df, max_samples=args.test, resume=False)
    elif args.limit:
        print(f"\n🎯 限制模式: 處理 {args.limit} 筆")
        run_backtest(calendar_df, max_samples=args.limit, resume=args.resume)
    elif args.full:
        print("\n🚀 完整回測模式")
        run_backtest(calendar_df, max_samples=None, resume=args.resume)
    else:
        parser.print_help()
